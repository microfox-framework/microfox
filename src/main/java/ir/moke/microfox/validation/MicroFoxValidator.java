...
public class MicroFoxValidator {
    ...
    public static <T> void validate(T t, Locale locale) {
        ...
        try (ValidatorFactory factory = Validation.byDefaultProvider().configure().messageInterpolator(messageInterpolator).buildValidatorFactory()) {
    ...
    private static MessageInterpolator getMessageInterpolator(Locale locale) {
        PlatformResourceBundleLocator resourceBundleLocator = new PlatformResourceBundleLocator(MicroFoxConfig.MICROFOX_RESOURCE_BUNDLE_NAME, Set.of(Locale.US, locale));
